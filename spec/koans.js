...
describe('`let` restricts the scope of the variable to the current block - ', () => {
  describe('`let` vs. `var`.', () => {
    ...
    it('`let` restricts scope to inside the block', () => {
      ...
      var letX;
      if (true) {
        var letX = false/*var or const? letX = true*/
      }
...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      ...
      const findSome = 'xyz'
      expect(findSome.includes('xyz')).toBe(true);
    });
  });
  ...
  describe('takes a position from where to start searching', function() {
    it('does not find `a` after position 1 in `abc`', function() {
      /*....*/
      const a = 'abc'
      expect(a.includes('a', 1)).toBe(false);
    });
    it('even the position gets coerced', function() {
      /*const findAtPosition = (pos) => 'xyz'.includes(?????);*/
      const findAtPosition = 'xyz'
      expect(findAtPosition.includes('z',2)).toBe(true);
    });
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        /*const findAtPosition = (pos) => 'xyz'.includes(?????); */
        const findAtPosition = 'xyz'
        expect(findAtPosition.includes('z',undefined)).toBe(true);
      });
      it('negative numbers', function() {
        /*const findAtPosition = (pos) => 'xyz'.includes(????); */
        
        const findAtPosition = 'xyz'
        expect(findAtPosition.includes('x',-2)).toBe(true);
      });
      it('NaN', function() {
        /* const findAtPosition = (pos) => 'xyz'.includes(?????); */
        const findAtPosition = 'xyz'
        expect(findAtPosition.includes('x', 'a')).toBe(true);
      });
    });
  });
...
describe('class creation', () => {
  ...
  it('multiple methods need no commas (opposed to object notation)', function() {
    class User {
      wroteATest() { return this.everWroteATest = true; }
