...
describe("arrow functions. ", () => {
  ...
  class LexicallyBound {
    getFunction() {
      return () => {
        return this;
        // new LexicallyBound(); /*changes might go here*/
      };
    }
  ...
  describe("arrow functions have lexical `this`, no dynamic `this`", () => {
    ...
    xit("can NOT bind a different context", function () {
      let bound = new LexicallyBound();
      let fn = bound.getFunction();
      let anotherObj = {};
...
describe("assign object property values to new variables while destructuring. ", () => {
  describe("for simple objects", function () {
    xit("use a colon after the property name, like so `propertyName: newName`", () => {
      const { x } = { x: 1 };
      //expect(y).toEqual(1);
    });

    xit("assign a new name and give it a default value using `= <default value>`", () => {
      const { x } = { y: 23 };
      //expect(y).toEqual(42);
    });
  });
  ...
  describe("for function parameter names", function () {
    xit("do it the same way, with a colon behind it", () => {
      const fn = ({ x }) => {
        //expect(y).toEqual(1);
      };
      fn({ x: 1 });
    ...
    xit("giving it a default value is possible too, like above", () => {
      const fn = ({ x }) => {
        //expect(y).toEqual(3);
      };
      fn({});
...
describe("spread with arrays. ", () => {
  xit("extracts each array item", function () {
    const [] = [...[1, 2]];
    //expect(a).toEqual(1);
    //expect(b).toEqual(2);
  });

  xit("in combination with rest", function () {
  ...
  xit("spreading into the rest", function () {
    const [...rest] = [...[, 1, 2, 3, 4, 5]];
    //expect(rest).toEqual([1, 2, 3, 4, 5]);
  });

  describe("used as function parameter", () => {
    xit("prefix with `...` to spread as function params", function () {
      const magicNumbers = [];
...
describe("spread with strings", () => {
  xit("simply spread each char of a string", function () {
    const [b, a] = ["ba"];
    //expect(a).toEqual('a');
    //expect(b).toEqual('b');
  });

  xit("works anywhere inside an array (must not be last)", function () {
...
describe("class creation", () => {
  xit("is as simple as `class XXX {}`", function () {
    let TestClass = {};
  ...
  xit("special method is `constructor`", function () {
    class User {
      constructor(id) {}
    }
  ...
  xit("defining a method is simple", function () {
    class User {}
  ...
  xit("multiple methods need no commas (opposed to object notation)", function () {
    class User {
      wroteATest() {
