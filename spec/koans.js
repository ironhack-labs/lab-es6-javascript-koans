...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function () {
    it('that matches exactly', function () {
      function findSome(str) { return 'xyz' === str; }
  ...
  describe('takes a position from where to start searching', function () {
    ...
    describe('invalid positions get converted to 0', function () {
      ...
      it('NaN', function () {
        const findAtPosition = (pos) => 'xyz'.includes(false);
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('chained assignments', () => {
    let c, d;
    //let a, b = c, d = [1, 2];  //entiendo que debería de ser así...
...
describe('destructuring objects. ', () => {
  ...
  describe('nested', () => {
    it('multiple objects', () => {
      const magic = { first: 23, second: 42 }; //esto debería de bastar
      //const first, second = magic;
      expect(second).toEqual(42);
    });
    it('object and array', () => {
      const { z: x } = { z: [23, 42] }; //no entiendo el funcioamiento
      expect(x).toEqual(42);
    });
    it('array and object', () => {
      const lang = [null, [{ env: 'browser', lang: 'ES6' }]];  //no sé llegar al campo lang...
      lang = lang[[{ lang }]]
  ...
  describe('interesting', () => {
    it('missing refs become undefined', () => {
      const z = { x: 1, y: 2, z: undefined };  //no entiendo.
...
describe('arrow functions. ', () => {
  ...
  describe('arrow functions have lexical `this`, no dynamic `this`', () => {
    ...
    it('bound at definition time, use `=>` ', function () {
      ...
      expect(fn()).toBe(bound);  //no entiendo que espera
    });
...
describe('destructuring function parameters. ', () => {
  ...
  describe('destruct parameters', () => {
    ...
    it('multiple params from array/object', () => {
      ...
      //const [{ name }, { name, id }] = users   //problems
      fn(users);
    });
  });
...
describe('assign object property values to new variables while destructuring. ', () => {
  ...
  describe('for simple objects', function () {
    it('use a colon after the property name, like so `propertyName: newName`', () => {
      const { y } = `{ x: 1 }`;  //no entiendo
      expect(y).toEqual(1);
    });

    it('assign a new name and give it a default value using `= <default value>`', () => {
      //const { x } = { name='< Paco >', y: 23 };  //no entiendo
      expect(y).toEqual(42);
    });
  });
...
describe('spread with arrays. ', () => {
  ...
  it('in combination with rest', function () {
    const [first, a, b, ...rest] = [...[0, 1, 2, 3, 4, 5]];
    expect(a).toEqual(1);
    expect(b).toEqual(2);
...
describe('class creation', () => {
  ...
  it('class is block scoped', () => {
    class Inside { }
    { class Inside { undefined } }
    expect(typeof Inside).toBe('undefined');  //no entiendo
  });
  ...
  it('multiple methods need no commas (opposed to object notation)', function () {
    ...
    tester.wroteATest();
    expect(tester.isLazy()).toBe(false); //no entiendo porque da error
  });
