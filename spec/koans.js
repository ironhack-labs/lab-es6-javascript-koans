...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('takes a position from where to start searching', function() {
    ...
    it('even the position gets coerced', function() {
      const findAtPosition = (pos) => 'xyz'.includes('xyz'.charAt(pos));
      expect(findAtPosition('2')).toBe(true);
    });
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        const findAtPosition = pos => "xyz".includes(pos === undefined ? "xyz".charAt(0) : "xyz".charAt(pos));
        expect(findAtPosition(void 0)).toBe(true);
      });
      it('negative numbers', function() {
        const findAtPosition = pos => "xyz".includes("xyz".charAt(Math.abs(pos))); 
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
        const findAtPosition = (pos) => 'xyz'.includes(pos === NaN ? "xyz".charAt(0) : "xyz".charAt(pos));
...
describe('arrow functions. ', () => {
  ...
  class LexicallyBound {
    // Using arrow notation, the 'this' gets passed on from return () towards the getFuncion(), 
    // towards the instance of LexicallyBound.
    ...
    // Here, a this will be restricted (or scoped) to the return function (), as such you'll 
    // need a 'let blabla = this' outside of the return function() to access its own instance, or self.
...
describe('destructuring function parameters. ', () => {
  ...
  describe('destruct parameters', () => {
    it('multiple params from object', () => {
      ...
      const {name, name: user, id} = {name: 'Wolfram', id: 42};
      fn(user);
    });
    ...
    it('multiple params from array/object', () => {
      ...
      const [,{name, name: users}] = [{name: 'nobody'}, {name: 'Alice', id: 42}];
      fn(users);
    });
  });
