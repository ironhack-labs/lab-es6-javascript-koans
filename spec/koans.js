...
describe("`let` restricts the scope of the variable to the current block - ", () => {
    describe("`let` vs. `var`.", () => {
        ...
        it("`let` restricts scope to inside the block", () => {
            let letX = false;
            if (true) {
                // redeclaring variable with var always drops an error
            }
...
it("`let` restricts scope to inside the block also in `for` loops", () => {
    let counter = 100;
    //first let declaration has block scope and second one has for scope
...
describe("`const` is like `let` plus read-only. ", () => {
    ...
    describe("complex types are NOT fully read-only", () => {
        it("arrays is not fully read-only", () => {
            const arr = [42, 23];
            arr[0] = 0
        //Immunatibilty only counts for primtives, not for complex types
...
describe("spread with arrays. ", () => {
    ...
    it("in combination with rest", function () {
        const [...[num1, a, b, ...rest]] = [...[0, 1, 2, 3, 4, 5]];
        expect(a).toEqual(1);
        expect(b).toEqual(2);
...
describe("class creation", () => {
    it("is as simple as `class XXX {}`", function () {
        let TestClass = class {constructor(){
