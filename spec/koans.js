  ...
  describe('takes a position from where to start searching', function() {
    ...
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        const findAtPosition = (pos) => {if((arguments.length >1) || (!pos)) pos =0;
          return 'xyz'.includes("xyz"[pos]);}
        expect(findAtPosition(void 0)).toBe(true);
      });
      it('negative numbers', function() {
        const findAtPosition = (pos) => {if((pos < 0) || (!pos)) pos = 0;
          return 'xyz'.includes("xyz"[pos]);}
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
        const findAtPosition = (pos) => {if(isNaN(pos)) pos = 0
          return 'xyz'.includes("xyz"[pos])}; 
