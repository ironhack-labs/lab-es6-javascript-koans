...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      const findSome = xyz => 'xyz'.includes(xyz);
  ...
  describe('takes a position from where to start searching', function() {
    ...
    describe('invalid positions get converted to 0', function() {
      ...
      const findAtPosition = (pos) => 'xyz'.includes('x'/* no entiendo esta wea xD*/);
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
      const findAtPosition = (pos) => 'xyz'.includes('y'/*la misma wea :v por que funciona sin el pos¿¿??*/);
...
describe('destructuring function parameters. ', () => {
  ...
  describe('default values', () => {
    ...
    it('for a missing array value', () => {
      const defaultUser = {id: 23, name: 'Joe'}//i't me :'D;
      const fn = ([user]) => {
        user = defaultUser
