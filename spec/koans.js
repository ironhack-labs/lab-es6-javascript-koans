...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      const findSome = (search) => 'xyz'.includes(search);
...
class LexicallyBound {
  ...
  describe('arrow functions have lexical `this`, no dynamic `this`', () => {
    ...
    it('bound at definition time, use `=>` ', function() {
      ...
      expect(fn()).toBe(bound); //que paso aqui?!!----------!!
    });
    ...
    it('can NOT bind a different context', function() {
      ...
      expect(fn.call(anotherObj)).toBe(expected); //NO ENTIENDO ESTO----!!!
    });
    ...
    it('`arguments` doesnt work inside arrow functions', function() {
      ...
      expect(fn(1, 2).length).toEqual(0);   //TMAPOCO ESTO-----!!!--------!!!
    });
...
describe('destructuring function parameters. ', () => {
  ...
  describe('destruct parameters', () => {
    it('multiple params from object', () => {
      ...
      const {name, id} = {name: 'Wolfram', id: 42};
      fn(name, id);
    });
    ...
    it('multiple params from array/object', () => {
      ...
      const [,{name, id}] = [{name: 'nobody'}, {name: 'Alice', id: 42}];
      fn(name, id);
    });
  });
...
describe('assign object property values to new variables while destructuring. ', () => {
  ...
  describe('for simple objects', function() {
    ...
    it('assign a new name and give it a default value using `= <default value>`', () => {
      const {x:y2=42, y} = {y: 23}; //Creo que eso no era -----!-----!
      expect(y2).toEqual(42);
    });
  });
