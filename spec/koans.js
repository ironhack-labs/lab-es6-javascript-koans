  ...
  describe('takes a position from where to start searching', function() {
    ...
    describe('invalid positions get converted to 0', function() {
      ...
      it('negative numbers', function() {
        const findAtPosition = (pos) => 'xyz'.includes('y', pos); //No entendi, Siempre da true
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
        const findAtPosition = (pos) => 'xyz'.includes('x', pos); //No entendi, siempre da true
...
describe('The object literal allows for new shorthands. ', () => {
  ...
  describe('with methods', () => {
    ...
    it('using the name only uses it as key', () => { //No entendi
      let short = {func};
...
describe('destructuring function parameters. ', () => {
  ...
  describe('destruct parameters', () => {
    ...
    it('multiple params from array/object', () => {
      const fn = ([,{name}]) => {     // No entendi, Tengo que usar la , para que pase del primer elemento??
  ...
  describe('default values', () => {
    ...
    it('mix of parameter types', () => {
      const fn = (id = 1, [arr = 2], {obj = 3}) => { //No entendi, esto debe ser así??
        expect(id).toEqual(1);
...
describe('assign object property values to new variables while destructuring. ', () => {
  ...
  describe('for simple objects', function() {
    it('use a colon after the property name, like so `propertyName: newName`', () => {
      const {x:y} = {x: 1}; //Es correcto??
      expect(y).toEqual(1);
    });

    it('assign a new name and give it a default value using `= <default value>`', () => {
      const {x:y=42} = {y: 23}; //No entendi como funciona?
      expect(y).toEqual(42);
    });
  });
...
describe('spread with arrays. ', () => {
  ...
  describe('used as function parameter', () => {
    it('prefix with `...` to spread as function params', function() {
      ...
      fn(magicNumbers); //Cómo funciona esto???
    });
  });
});
