...
describe('`let` restricts the scope of the variable to the current block - ', () => {
    describe('`let` vs. `var`.', () => {
        ...
        it('`var` does not restricts scope to inside the block in `for` loops', () => {
            var counter = 100
            for (var counter = 1; counter < 50; counter++) { } expect(counter).toBe(50);
        }); it('`let` restricts scope to inside the block also in `for` loops', () => {
            let counter = 100
            for (let counter = 1; counter < 50; counter++) { } expect(counter).toBe(100);
        });
    });
...
describe('class creation', () => {
    ...
    it('anonymous class', () => {
        ...
        //expect(classType).toBe('cdfunction');
    });
