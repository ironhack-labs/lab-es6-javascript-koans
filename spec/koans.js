  ...
  describe("takes a position from where to start searching", function() {
    ...
    describe("invalid positions get converted to 0", function() {
      ...
      it("negative numbers", function() {
        const findAtPosition = pos => "xyz".includes("z", Math.abs(pos));
        expect(findAtPosition(-2)).toBe(true);
      });
      it("NaN", function() {
        const findAtPosition = (pos = 0) => "xyz".includes("x", pos);
...
describe("destructuring arrays makes shorter code. ", () => {
  ...
  it("extract from nested arrays", () => {
    ...
    const [newArray, age] = user;
    const [firstName, surname] = newArray;
    const expected = "Some One = 23 years";
    expect(`${firstName} ${surname} = ${age} years`).toEqual(expected);

    //let a = [first, last, age2] = [...["Some", "One"], 23];
  });
...
describe("destructuring objects. ", () => {
  ...
  describe("nested", () => {
    ...
    it("array and object", () => {
      const [x, [{ y, lang }]] = [null, [{ env: "browser", lang: "ES6" }]];
