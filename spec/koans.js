...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('takes a position from where to start searching', function() {
    ...
    it('even the position gets coerced', function() {
      const findAtPosition = (pos) => 'xyz2'.includes("2");
      expect(findAtPosition('2')).toBe(true);
    });
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        const findAtPosition = (pos) => 'xyz0'.includes("0"); 
        expect(findAtPosition(void 0)).toBe(true);
      });
      it('negative numbers', function() {
        const findAtPosition = (pos) => 'xyz-2'.includes("-2"); 
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
        const findAtPosition = (pos) => 'xyzNaN'.includes(NaN); 
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('extract from nested arrays', () => {
    ...
    expect("Some One = 23 years").toEqual(expected);
  });
...
describe('arrow functions. ', () => {
 ...
 describe('arrow functions have lexical `this`, no dynamic `this`', () => {
    ...
    it('`arguments` doesnt work inside arrow functions', function() {
      let bound = new LexicallyBound();
      let fn = (x,y) => bound.getArgumentsFunction();
