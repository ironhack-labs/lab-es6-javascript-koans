...
describe("`let` restricts the scope of the variable to the current block - ", () => {
  describe("`let` vs. `var`.", () => {
    ...
    it("`let` restricts scope to inside the block also in `for` loops", () => {
      /*var or let? counter = 100*/
      let counter = 100;
      /*for (var or let? counter = 1; counter < 50; counter++){}0*/
...
describe("`const` is like `let` plus read-only. ", () => {
  ...
  describe("complex types are NOT fully read-only", () => {
    it("arrays is not fully read-only", () => {
      const arr = [42, 23];
      arr.splice(0, 0, 0); //not sure that this is what i suppose to do, it works though!!
      expect(arr[0]).toBe(0);
    });
...
describe("arrow functions. ", () => {
  ...
  it("one parameter can be written without parens", () => {
    /* let func = ........;*/
    //let func = num => num = num--;  WHY IS NOT WORKING!!
  ...
  describe("arrow functions have lexical `this`, no dynamic `this`", () => {
    ...
    it("can NOT bind a different context", function() {
      ...
      let fn = bound.getFunction();
      let anotherObj = {};
      let expected = bound; //change this NOT REALLY SURE WHY!!!
