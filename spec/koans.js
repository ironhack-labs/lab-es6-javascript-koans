...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('takes a position from where to start searching', function() {
    ...
    it('even the position gets coerced', function() {
      const findAtPosition = (pos) => 'xyz'.includes("zyx"[pos]);
      expect(findAtPosition('2')).toBe(true);
    });
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        const findAtPosition = pos => "xyz".includes("zyx"[pos]); 
        expect(findAtPosition(0)).toBe(true);
      });
      it('negative numbers', function() {
        const findAtPosition = (pos) => 'xyz'.includes("zyx"[pos]);
        expect(findAtPosition(-2)).toBe(false); //cambie x false 
      });
      it('NaN', function() {
        const findAtPosition = pos => "xyz".includes("xyz");
        expect(findAtPosition(NaN)).toBe(true); //como valido que no sean numeros?
      });
    });
  });
...
describe('The object literal allows for new shorthands. ', () => {
  ...
  describe('with methods', () => {
    ...
    it('a different key must be given explicitly, just like before ES6', () => {
      ...
      expect(short).toEqual({otherKey: func});//MISTAKE?
    });
  });
});
...
describe('class creation', () => {
  ...
  it('class is block scoped', () => {
    class Inside {}
    { class Inside2 {} }
    expect(typeof Inside2).toBe('undefined');
  });
  ...
  it('multiple methods need no commas (opposed to object notation)', function() {
    ...
    expect(tester.isLazy(true)).toBe(true);
    tester.wroteATest();
    expect(tester.isLazy(false)).toBe(false);
  });
