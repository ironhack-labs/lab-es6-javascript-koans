...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('scalar values are read-only', () => {
    it('number are read-only', () => {
      const constNum = 0;
      // constNum = 1; --> Throws error of assignment to a constant variable
    ...
    it('string are read-only', () => {
      ...
      // constString = "Cant change you?"; --> Same as above
...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      const findSome = (stringToBeFound) => 'xyz'.includes(stringToBeFound)
  ...
  describe('takes a position from where to start searching', function() {
    ...
    // We're not sure about the following 3 solutions
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('extract value from array, e.g. extract 0 into x like so `let [x] = [0];`', () => {
    let firstValue = [1];
    let [firstValueDestructured] = firstValue 
    expect(firstValueDestructured).toEqual(1);
  });
