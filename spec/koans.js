...
describe('`let` restricts the scope of the variable to the current block - ', () => {
  describe('`let` vs. `var`.', () => {
    ...
    it('`let` restricts scope to inside the block', () => {
      /*var or const? letX = false*/ const letX = false
      if (true) {
        /*var or const? letX = true*/ const letX = true
      }
      expect(letX).toBe(false)
    })

    it('`var` does not restricts scope to inside the block in `for` loops', () => {
      /*var or let? counter = 100*/ var counter = 100
      /*for (var or let? counter = 1; counter < 50; counter++){}*/ for (
        var counter = 1;
        counter < 50;
        counter++
      ) {}
    ...
    it('`let` restricts scope to inside the block also in `for` loops', () => {
      /*var or let? counter = 100*/ let counter = 100
      /*for (var or let? counter = 1; counter < 50; counter++){}*/ for (
        let counter = 1;
        counter < 50;
        counter++
...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function () {
    it('that matches exactly', function () {
      ...
      const findSome = (char) => 'xyz'.includes(char)
  ...
  describe('takes a position from where to start searching', function () {
    ...
    describe('invalid positions get converted to 0', function () {
      it('e.g. `undefined`', function () {
        /*const findAtPosition = (pos) => 'xyz'.includes(?????); */
        const findAtPosition = (pos) => 'xyz'.includes('x', undefined)
...
describe('arrow functions. ', () => {
  ...
  describe('arrow functions have lexical `this`, no dynamic `this`', () => {
    ...
    it('can NOT bind a different context', function () {
      ...
      let fn = bound.getFunction()
      let anotherObj = {}
      let expected = fn(anotherObj) //change this -- puede ser bound
  ...
  it('multiple methods need no commas (opposed to object notation)', function () {
    class User {
      wroteATest() {
        ...
        if (this.everWroteATest) return false
      }
      isLazy() {
        if (!this.everWroteATest) return true
        else return false
      }
    }
