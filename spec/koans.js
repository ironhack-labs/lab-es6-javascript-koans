...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('scalar values are read-only', () => {
    it('number are read-only', () => {
      const constNum = 0;
      constNum; // not sure this question is asking
      // it of course throws a type error, rather than failing silently like the test seems to expect
    ...
    it('string are read-only', () => {
      ...
      // throws type error, rather than failing silently like the test seems to expect
...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      const findSome = (subStr) => 'xyz'.includes(subStr);
...
describe('arrow functions. ', () => {
  ...
  describe('arrow functions have lexical `this`, no dynamic `this`', () => {
    ...
    it('can NOT bind a different context', function() {
      ...
      let fn = bound.getFunction();
      let anotherObj = {};
      let expected = fn(anotherObj); // ? what, i solved it but surely not how they intended?
