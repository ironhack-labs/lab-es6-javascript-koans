...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('scalar values are read-only', () => {
    it('number are read-only', () => {
      ...
      expect(function(){constNum = 1}).toThrowError("Assignment to constant variable.");
    });

    it('string are read-only', () => {
      const constString = "I am a const";
      expect(constString).toBe("I am a const");
      expect(function(){constString = "Can't change you"}).toThrowError("Assignment to constant variable.");
    });
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('extract from nested arrays', () => {
    ...
    const[firstName,surname,age=23]=user[0];
