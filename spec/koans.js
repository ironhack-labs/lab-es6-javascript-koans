...
describe("`let` restricts the scope of the variable to the current block - ", () => {
  describe("`let` vs. `var`.", () => {
    it("`var` works as usual, it does not restrict scope", () => {
      if (true) {
        var varX = true;
      }
...
describe("arrow functions. ", () => {
  ...
  it("one parameter can be written without parentheses", () => {
    let func = x => x - 1;
  ...
  describe("arrow functions have lexical `this`, no dynamic `this`", () => {
    ...
    it("`arguments` don't work inside arrow functions", function() {
      let bound = new LexicallyBound();
...
describe("destructuring objects. ", () => {
  ...
  describe("nested", () => {
    ...
    it("array and object", () => {
      const [none, [{ env, lang }]] = [null, [{ env: "browser", lang: "ES6" }]];
...
describe("spread with arrays. ", () => {
  ...
  it("in combination with rest", function() {
    const [...[something, a, b, ...rest]] = [...[0, 1, 2, 3, 4, 5]];
    expect(a).toEqual(1);
    expect(b).toEqual(2);
