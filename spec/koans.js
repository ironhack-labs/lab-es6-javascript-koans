...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('scalar values are read-only', () => {
    it('number are read-only', () => {
      const constNum = 0;
      //constNum = 1; no se puede reasignar en este caso
    ...
    it('string are read-only', () => {
      ...
      //constString = "Cant change you?"; No, I can't
...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a single character', function() {
    it('in a three char string', function() {
      ...
      expect('xyz'.includes(searchString)).toBe(true); //preguntar <---------------
    });
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('leading commas', () => {
    const all = ['ax', 'why', 'zet']; // preguntar 
    const [z] = [all[2]];
...
describe('destructuring also works on strings. ', () => {
  ...
  it('missing characters are undefined', () => {
    const [a, c] = 'a';
    expect(c).toEqual(void 0);  //preguntar <------------
  });
});
