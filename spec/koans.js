...
describe("`let` restricts the scope of the variable to the current block - ", () => {
  describe("`let` vs. `var`.", () => {
    ...
    it("`let` restricts scope to inside the block", () => {
      /*var or const?*/ const letX = false;
      if (true) {
        /*var or const?*/ const letX = true;
      }
      expect(letX).toBe(false);
    });

    it("`var` does not restricts scope to inside the block in `for` loops", () => {
      /*var or let?*/ var counter = 100;
      for (/*var or let?*/ var counter = 1; counter < 50; counter++) {}

      expect(counter).toBe(50);
    });

    it("`let` restricts scope to inside the block also in `for` loops", () => {
      /*var or let?*/ let counter = 100;
      for (/*var or let?*/ let counter = 1; counter < 50; counter++) {}
...
describe("`const` is like `let` plus read-only. ", () => {
  ...
  /*var, let or const?*/ var notChangeable = 23;
...
describe("`string.includes()` finds string within another string. ", () => {
  describe("find a single character", function() {
    ...
    it("reports false if character was not found", function() {
      /* const expected = ????*/ const expected = false;
  ...
  describe("takes a position from where to start searching", function() {
    ...
    describe("invalid positions get converted to 0", function() {
      ...
      it("negative numbers", function() {
        const findAtPosition = pos => "xyz".includes("yz", pos);
        expect(findAtPosition(-2)).toBe(true);
      });
      it("NaN", function() {
