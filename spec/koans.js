...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('scalar values are read-only', () => {
    ...
    it('string are read-only', () => {
      ...
      expect(constString).toBe("Cant change you?");
    });
...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('takes a position from where to start searching', function () {
    it('does not find `a` after position 1 in `abc`', function () {
      /*....*/
      expect('abc'.includes('a', 2)).toBe(false);
    });
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('swap two variables, in one operation', () => {
    let [x, y] = ['ax', 'why'];
    [x, y] = [x, y];
    expect([x, y].reverse()).toEqual(['why', 'ax']);
  });

  it('leading commas', () => {
    const all = ['ax', 'why', 'zet'];
    const [z] = all;
    expect(all[2]).toEqual('zet');
  });
  ...
  it('chained assignments', () => {
    let c, d;
    let a, b = c = [1, 2];
    expect([b[0], b[1], b[0], b[1]]).toEqual([1, 2, 1, 2]);
  });
...
describe('destructuring objects. ', () => {
  ...
  describe('nested', () => {
    it('multiple objects', () => {
      ...
      const first = 23, second = magic.second
