...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('complex types are NOT fully read-only', () => {
    ...
    it('arrays is not fully read-only', () => {
      const arr = [0, 23]; // Aquí modifiqué el 42 por 0, no estoy seguro si es la respuesta acertada

      expect(arr[0]).toBe(0);
    });

    it('objects are not fully read-only', () => {
      const obj = {x: 2}; // Aquí modifiqué el 1 por 2, no estoy seguro si es la respuesta acertada
