  ...
  describe("`let` vs. `var`.", () => {
    ...
    it("`let` restricts scope to inside the block", () => {
      ...
      if (true) {
        ...
        //->neither, both would cause an error
      }
...
describe("`const` is like `let` plus read-only. ", () => {
  describe("scalar values are read-only", () => {
    it("number are read-only", () => {
      const constNum = 0;
      // constNum = 1; --> cant assignt to an only-readable variable
...
describe("`string.includes()` finds string within another string. ", () => {
  ...
  describe("takes a position from where to start searching", function () {
    ...
    describe("invalid positions get converted to 0", function () {
      ...
      it("NaN", function () {
        const findAtPosition = (pos) => "xyz".includes("x" || "y" || "z", pos);
...
describe("arrow functions. ", () => {
  ...
  it("one parameter can be written without parens", () => {
    let func = (number) => number - 1; // on save, PRETTIER puts parens
