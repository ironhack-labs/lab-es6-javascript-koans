...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      const findSome = (x) => 'xyz'.includes(x);
  ...
  describe('takes a position from where to start searching', function() {
    it('does not find `a` after position 1 in `abc`', function() {
      const position = 1 //?? no empieza en 0: invalid positions get converted to 0
...
describe('The object literal allows for new shorthands. ', () => {
  ...
  describe('with methods', () => {
    ...
    it('a different key must be given explicitly, just like before ES6', () => {
      ...
      expect(short).toEqual({otherKey: func});//wtf??
    });
  });
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('extract value from array, e.g. extract 0 into x like so `let [x] = [0];`', () => {
    let firstValue = 1;
    expect(firstValue).toEqual(1); //wtf??
  });
  ...
  it('chained assignments', () => {
    ...
    a = c = 1
    b = d = 2
    expect([a, b, c, d]).toEqual([1, 2, 1, 2]);//wtf
  });
...
describe('arrow functions. ', () => {
  ...
  class LexicallyBound {
    ...
    getFunction() {
      return () => this //No tengo ni pta idea de porque va esto asi. 
      // return () => {
  ...
  describe('arrow functions have lexical `this`, no dynamic `this`', () => {
    ...
    it('can NOT bind a different context', function() {
      ...
      let fn = bound.getFunction();
      let anotherObj = {};
      let expected = bound; //change this //ni puta idea
...
describe('spread with arrays. ', () => {
  ...
  it('spreading into the rest', function() {
    ...
    expect(rest).toEqual([1, 2, 3, 4, 5]); //no entiendo esta mierda
  });
...
describe('class creation', () => {
  ...
  it('multiple methods need no commas (opposed to object notation)', function() {
    class User {
      ...
      isLazy()     { 
        return this.everWroteATest == true ? false : true
      }
    }
