  ...
  describe('takes a position from where to start searching', function() {
    ...
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        const findAtPosition = (pos) => pos && pos >= 0 && pos <= 2? 'xyz'.includes("z", pos) : true
        expect(findAtPosition(void 0)).toBe(true);
      });
      it('negative numbers', function() {
        const findAtPosition = (pos) => pos >= 0? 'xyz'.includes(): true
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
        const findAtPosition = (pos) => pos? 'xyz'.includes() : true
...
describe('destructuring function parameters. ', () => {
  ...
  describe('default values', () => {
    ...
    it('for a missing array value', () => {
      ...
      const fn = ([user]) => { return user ? user : defaultUser
