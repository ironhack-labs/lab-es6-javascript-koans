  ...
  describe('takes a position from where to start searching', function() {
    ...
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        const findAtPosition = (pos) => 'xyz'.includes('xy', pos);
        expect(findAtPosition(void 0)).toBe(true);
      });
      it('negative numbers', function() {
        const findAtPosition = (pos) => 'xyz'.includes('x', pos); // @LEARN don't understand why every letter works here! :)
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
...
describe('destructuring also works on strings. ', () => {
  ...
  it('destructure every character', () => {
    let [a, b, c] = 'abc'; // @REMEMBER

    expect([a, b, c]).toEqual(['a', 'b', 'c']);
  });

  it('missing characters are undefined', () => {
    const [a, , c] = 'ab'; // @REMEMBER
    // const [a, b, c] = 'ab'; works, too!
...
describe('destructuring objects. ', () => {
  ...
  describe('nested', () => {
    ...
    it('object and array', () => {
      const { z:[, x ]} = {z: [23,42]}; // @REMEMBER omit first object (23) with `,`

      expect(x).toEqual(42);
    });
    it('array and object', () => {
      const [, [{lang}]] = [null, [{env: 'browser', lang: 'ES6'}]]; // @REMEMBER: 1. array, 2. omit null, 3. array, 4. object hash => lang
...
describe('arrow functions. ', () => {
  ...
  it('one parameter can be written without parens', () => {
   /* let func = ........;*/
   let func = num => num - 1;
   // let func = num => { return num - 1}; // @REMEMBER works, too! Using curly braces requires to return the value
  ...
  describe('arrow functions have lexical `this`, no dynamic `this`', () => {
    ...
    it('can NOT bind a different context', function() {
      let bound = new LexicallyBound(); // instantiate new class
      let fn = bound.getFunction(); // access LexicallyBound.getFunction via bound
      let anotherObj = {}; // Initialize and assign empty hash
      let expected = bound; // Assign all bound functions and properties to expected variable
...
describe('assign object property values to new variables while destructuring. ', () => {
  ...
  describe('for simple objects', function() {
    it('use a colon after the property name, like so `propertyName: newName`', () => {
      const {x: y} = {x: 1}; // @REMEMBER

      expect(y).toEqual(1);
    });

    it('assign a new name and give it a default value using `= <default value>`', () => {
      const {x: y = 42} = {y: 23}; // @REMEMBER
...
describe('spread with strings', () => {
  ...
  it('simply spread each char of a string', function() {
    const [b, a] = [...'ba'];
    expect(a).toEqual('a'); // @REMEMBER b => 'b' of `...ba`
    expect(b).toEqual('b'); // @REMEMBER a => 'a' of `...ba`
  });

  it('works anywhere inside an array (must not be last)', function() {
    const letters = ['a', ...'bcd', 'e', 'f']; // @REMEMBER 'a' (1), 'bcd' (3), 'e' (1), 'f' (1)
...
describe('class creation', () => {
  ...
  it('anonymous class', () => {
    const classType = typeof class {}; // get typeof class and assign to classType
