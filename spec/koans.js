...
describe('assign object property values to new variables while destructuring. ', () => {
  ...
  describe('for simple objects', function() {
    it('use a colon after the property name, like so `propertyName: newName`', () => {
      const {x:y} =  {x:1};//PREGUNTAR:A que se refiere con poner la comilla????? please JUAN!thanks
      expect(y).toEqual(1);
    });
  ...
  describe('for function parameter names', function() {
    it('do it the same way, with a colon behind it', () => {
      const fn = ({x:y}) => { // PREGUNTARRRR es mas o menos lo de antes, no entiendo muy bien a que se refiere ocn lo de la comilla,pero asi funciona.
       expect(y).toEqual(1);
      };
      fn({x: 1});
    });
