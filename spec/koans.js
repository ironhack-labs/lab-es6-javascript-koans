...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('takes a position from where to start searching', function() {
    it('does not find `a` after position 1 in `abc`', function() {
      let fromPosition = 1
      expect('abc'.includes('a', fromPosition)).toBe(false);
    });
    it('even the position gets coerced', function() {
      /*const findAtPosition = (pos) => 'xyz'.includes(?????);*/
      let findAtPosition = pos => 'xyz'.includes('z', pos)
    ...
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        /*const findAtPosition = (pos) => 'xyz'.includes(?????); */
        let findAtPosition = pos => 'xyz'.includes('z', pos)
        //function findAtPositon(pos) {'xyz'.includes('z', pos)}
        expect(findAtPosition(undefined)).toBe(true);
      });
      it('negative numbers', function() {
        /*const findAtPosition = (pos) => 'xyz'.includes(????); */
        let findAtPosition = pos => 'xyz'.includes('x', pos)
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
        /* const findAtPosition = (pos) => 'xyz'.includes(?????); */
...
//Esto no lo entendimos muy bien, creemos que no estamos haciendo lo que se nos pide
describe('assign object property values to new variables while destructuring. ', () => {

  describe('for simple objects', function() {
    it('use a colon after the property name, like so `propertyName: newName`', () => {
      const {x} = {x: 1};
      expect(y=x).toEqual(1);
    });

    it('assign a new name and give it a default value using `= <default value>`', () => {
      const {x} = {y: 23};
      expect(y=42).toEqual(42);
    });
  });
