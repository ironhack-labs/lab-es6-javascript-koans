...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      const findSome = s => 'xyz'.includes(s);
...
describe('assign object property values to new variables while destructuring. ', () => {
  ...
  describe('for simple objects', function() {
    it('use a colon after the property name, like so `propertyName: newName`', () => {
      const {x, y} = {x: 1, y: 1}; // const {x} = {x: 1};
      expect(y).toEqual(1);
    });

    it('assign a new name and give it a default value using `= <default value>`', () => {
      const {x, y = 42} = {x: 23}; // const {x} = {y: 23};
      expect(y).toEqual(42);
    });
  });
...
describe('spread with strings', () => {
  ...
  it('simply spread each char of a string', function() {
    const [b, a] = ['b','a']; // const [b, a] = ['ba'];
    expect(a).toEqual('a');
    expect(b).toEqual('b');
  });

  it('works anywhere inside an array (must not be last)', function() {
    const letters = [,,'a', 'bcd', 'e', 'f']; // const letters = ['a', 'bcd', 'e', 'f'];
...
describe('class creation', () => {
  ...
  it('anonymous class', () => {
    const classType = typeof class Func{};
