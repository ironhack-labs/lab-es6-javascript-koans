...
describe('`let` restricts the scope of the variable to the current block - ', () => {
  describe('`let` vs. `var`.', () => {
    ...
    it('`let` restricts scope to inside the block', () => {
      let letX = false/*var or const? letX = false*/
      if (true) {
    ...
    it('`var` does not restricts scope to inside the block in `for` loops', () => {
      let counter = 50/*var or let? counter = 100*/
...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      let pepe = 'xyz'
      const findSome = pepe => 'xyz'.includes(pepe);
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('extract from nested arrays', () => {
    ...
    const [first,age] = user;
    const [firstName,surname] = first;
...
describe('destructuring also works on strings. ', () => {
  ...
  it('missing characters are undefined', () => {
    const [a, ,c] = 'ab'; //const [a,c] = 'a'; TAMBIEN FUNCIONA
