...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
      const findSome = findAString => 'xyz'.includes(findAString);
  ...
  describe('search for an empty string, is always true', function() {
    ...
    //si en un string buscas un string vacio siempre va a ser true
    it('in `abc`', function() {
      const letter = ''
      expect('abc'.includes(letter)).toBe(true);
    });
  });
...
describe('class creation', () => {
  ...
  it('multiple methods need no commas (opposed to object notation)', function() {
    class User {
      ...
      isLazy()     { 
        if(this.everWroteATest === true) return false;
        return true
       }
    }
